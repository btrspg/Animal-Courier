...
def main():
    ...
    cmds = shell_cmd.get_cmds(args.shell, work_log, args.work_name)

    if args.profile:
        from animalcourier.profile.psopen import pspopen
        all_infos = pool.starmap(pspopen, cmds, args.interval)
    else:
    ...
    if args.profile:
        ...
        for _, _, csv in all_infos:
            ...
            merge_data = data[data.columns[-1]] if isinstance(merge_data, str) else merge_data.join(data[data.columns[-1]],how='outer')
    print(merge_data)
